...
class WeatherDisplay
{
  ...
  protected:
    ...
    void DrawJsonData(int x, int y, int dy, Json_data &jsonData, uint8_t column_id);
...
void WeatherDisplay::DrawHead()
{
  ...
  canvas.drawCentreString(HEADLINE_STRING, maxX / 2, 10, 1);
...
{
  canvas.setTextSize(3);
  canvas.drawCentreString("Local", x + dx / 2, y + 7, 1);
  canvas.drawLine(x, y + 35, x + dx, y + 35, M5EPD_Canvas::G15);
...
#if defined(DISPLAY_WEATHER_DETAILS_DAILY)
